...
def extract_groq_prediction(response: dict, prompt_type: str) -> str:
    ...
    debug_log_path = script_dir / "debug_responses.log"
    ...
    try:
        with open(str(debug_log_path), "a", encoding="utf-8") as f:
            f.write(debug_msg)
        print(f"[DEBUG] Wrote to log: {debug_log_path}")
    ...
    for idx, each in enumerate(test_data):
        ...
        while True:
            try:
                ...
                print(f"\n[SAMPLE {idx + 1}] Raw response type: {type(response)}")
                print(f"[SAMPLE {idx + 1}] Raw response: {response}")
