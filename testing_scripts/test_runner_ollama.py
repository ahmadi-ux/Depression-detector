...
for each in dataset["test"]:
    ...
    if each["class"] == "0" and response.strip() == "depressed":
        TP += 1
        total_ran += 1
        print("Successfully predicted depressed for text: " + str(TP))
    elif each["class"] != "0" and response.strip() == "depressed":
        FP += 1
        total_ran += 1
        print("Incorrectly predicted depressed for text: " + str(FP))
    elif each["class"] != "0" and response.strip() == "not-depressed":
        TN += 1
        total_ran += 1
        print("Successfully predicted not-depressed for text: " + str(TN))
    elif each["class"] == "0" and response.strip() == "not-depressed":
        FN += 1
        total_ran += 1
        print("Incorrectly predicted not-depressed for text: " + str(FN))
    else:
