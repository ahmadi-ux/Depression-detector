...
def run_compound_job(file_payloads, prompt_type: str = "simple"):
    combined_results = []
    for payload in file_payloads:
        file = FileStorage(stream=io.BytesIO(payload["bytes"]), filename=payload["filename"])
        extracted_text = extract_text_from_file(file)
        compound_output = analyze_text(extracted_text, prompt_type)  # Pass prompt_type
        combined_results.append({
            "filename": payload["filename"],
            "text": extracted_text,
            "analysis": compound_output
        })
    pdf = generate_combined_pdf_report(combined_results, title_suffix="Compound")
    return pdf.getvalue()